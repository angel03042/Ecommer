...
fetch("https://script.google.com/macros/s/AKfycbwwf4MsmP0c1VyApxeAPzPXsyxg4mQ7ui7TOVL3jbrjJz00Pj9v0WgEFSAuIlCyKspq/exec")
  .then((r) => r.json())
  .then((data) => {
    ...
    // Mostrar secciones que estaban ocultas al inicio
        ...
        imgLike.addEventListener("click", () => {
          ...
          if (estaActivo) {
            ...
            if (!favoritos.some((p) => p.nombre === element.nombre)) {
              favoritos.push(element); // guardar todo el objeto
            }
          }
...
document.querySelector("#formDireccion").addEventListener("submit", (e) => {
  ...
  const direccionesGuardadas = JSON.parse(localStorage.getItem("direccion") || "[]");
...
// document.querySelector("#compraAceptada").addEventListener('click', () => document.querySelector(".listaCarrito").innerHTML = '')
